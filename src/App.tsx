...
function Logo({ footer = false }: LogoProps) {
  ...
  if (logoError) return <div className="flex items-center gap-3"><div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-gradient-to-br from-[#0F2D3A] via-[#16B3A3] to-[#F4C95D] text-lg font-black text-white shadow-sm">O</div><div><p className={`${footer ? "text-xl" : "text-lg"} font-black tracking-tight text-[#0F2D3A]`}>Octalumen</p><p className="-mt-1 text-xs font-medium text-slate-500">Clareza • Estrutura • Confiança</p></div></div>;
  return <img src={LOGO_SRC} alt="Octalumen" className={footer ? "h-26 w-auto object-contain" : "h-26 w-auto object-contain sm:h-30"} loading={footer ? "lazy" : "eager"} onError={() => setLogoError(true)} />;
}
function Header() {
